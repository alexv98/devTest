...
import { formatCharsInput } from "../helpers/maskFormat.js";
import maskIsValid from "../helpers/maskValidator.js";
...
class CodeEditor extends Component {
  ...
  render() {
    const { className, style, inputRef } = this.props; // Вытащил inputRef сразу из пропсов
...
class TextInputWithActions extends Component {
  constructor(props) {
    ...
    this.actionsNode = React.createRef(); // добавил ref
    this.changeTimer = null; // для таймера
    this.state = {
      actionsWidth: 0,
      value: props.value || "",
      oldValue: ""
  ...
  // Добавил метод жизненного цикла при размонтировании компонента
  componentWillUnmount() {
    clearTimeout(this.changeTimer); // Очищаем таймер
  }

  recalcActionsWidth() {
    if (!this.actionsNode?.current) {
      return;
    }

    const actionsWidth = this.actionsNode.current?.clientWidth; // достаем из элемента
    if ( actionsWidth && actionsWidth !== this.state.actionsWidth) {
      this.setState({
        actionsWidth
      });
    }
  ...
  onChange = e => {
    const value = e.target.value;
    // this.setState({ value }); // Дублировование кода. Повторяет код setValue
  ...
  setBlur = value => {
    ...
    this.setState({ oldValue: value }); // Изменение состояния через setState, вместо изменения oldValue напрямую
  };
    ...
    if (type === "number") {
      ...
      control = (
        <Select
          ...
          showSearch={true}
          variant={"filled"} // Заменил устаревший bordered
          suffixIcon={null} // Заменил устаревший showArrow
          popupMatchSelectWidth={300} // Заменил устаревший dropdownMatchSelectWidth
...
class UniversalInput extends Component {
  ...
  onChange = e => {
    this.props.onChange && this.props.onChange(e); // event, вместо value
